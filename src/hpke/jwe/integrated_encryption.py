...
def encrypt_integrated(public_key: str, plaintext: bytes) -> tuple[bytes, bytes]:
    ...
    ct = base64url_no_padding_encode(ciphertext)
    encrypted_key = base64url_no_padding_encode(encap)
    iv = ''
    tag = ''
...
def decrypt_integrated(private_key: str, jwe: str) -> bytes:
    ...
    aad = protected_header.encode(encoding='utf-8')
    encap = base64url_no_padding_decode(encrypted_key)
    ciphertext = base64url_no_padding_decode(ct)
